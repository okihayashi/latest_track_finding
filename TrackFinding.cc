...
int main(int argc, char** argv){
	...
	if(check_event>=15){
	    vector<double> CDCcell_layerID_cut(0);
            vector<double> CDCcell_cellID_cut(0);
	    vector<double> CDCcell_hittype_cut(0);
	    vector<double> CDCcell_px_signal(0);
	    vector<double> CDCcell_py_signal(0);
	    vector<double> CDCcell_pz_signal(0);
	    vector<double> CDCcell_x_signal(0);
	    vector<double> CDCcell_y_signal(0);
	    vector<double> CDCcell_z_signal(0);
		...
		vector<double> x_02(0);               //--* x-coordinate of hit wire that path through crossing cut 
		vector<double> y_02(0);               //--* y-coordinate of hit wire that path through crossing cut 
		vector<double> z_02(0);               //--* z-coordinate of hit wire that path through crossing cut(not use now) 
		vector<double> x_sig(0);              //--* x-coordinate of hit wire by signal 
		vector<double> y_sig(0);              //--* y-coordinate of hit wire by signal 
		vector<double> x_bg(0);               //--* x-coordinate of hit wire by noise 
		vector<double> y_bg(0);               //--* x-coordinate of hit wire by noise 
		...
		vector<double> signalNN_X(0);         //--* x-coordinate of hits selected by 1st neural network 
		vector<double> signalNN_Y(0);         //--* y-coordinate of hits selected by 1st neural network 
		vector<double> signalNN_X_2(0);       //--* x-coordinate of hits selected by 2nd neural network(not use now)  
		vector<double> signalNN_Y_2(0);       //--* y-coordinate of hits selected by 2nd neural network(not use now)  
		...
		vector<double> signalNN_X_cut(0);     //--* x-coordinate of hits path through the density cut
		vector<double> signalNN_Y_cut(0);     //--* y-coordinate of hits path through the density cut 
		...
		vector<double> signalNN_X_2_cut(0);   //--* x-coordinate of hits path through the 2nd density cut  
		vector<double> signalNN_Y_2_cut(0);   //--* y-coordinate of hits path through the 2nd density cut  
		...
		//--- Introduce 3D information ---------------------------------------------------------------------------------// 
                ...
                vector<double>   signalNN_X_2_cutEP(0);
                vector<double>   signalNN_Y_2_cutEP(0);
	       	vector<double>   signalNN_Z_2_cutEP(0);
		vector<double>   signalNN_X_2_cutEP2(0); 
		vector<double>   signalNN_Y_2_cutEP2(0); 
		vector<double>   signalNN_Z_2_cutEP2(0); 
		vector<int>      signalNN_layerID(0);
                vector<double>   signalNN_theta(0);
		vector<double>   signal_distance_X(0);
		vector<double>   signal_distance_Y(0);
		vector<double>   signal_distance_Z(0);
                vector<double>   signal_distance_X_cut(0); 
                vector<double>   signal_distance_Y_cut(0); 
                vector<double>   signal_distance_Z_cut(0);
                vector<double>   signal_distance_X_cluster(0);
                vector<double>   signal_distance_Y_cluster(0);
                vector<double>   signal_distance_Z_cluster(0); 
		...
		for(int i=0;i<x_sig.size();i++){                                             
			...
			if(fabs(x_sig[i]-signalNN_X_2_cut[j])<DBL_EPSILON && fabs(y_sig[i]-signalNN_Y_2_cut[j])<DBL_EPSILON){          
                ...
                for(int i=0;i<x_sig.size();i++){                                                                                           
                    for(int j=0;j<signal_distance_X_cut.size();j++){                                                                            
		...
		if(efficiency_4>=50. && (countsig_4/countbg_4)>=1.){
		    count_efficiency++;
		}
